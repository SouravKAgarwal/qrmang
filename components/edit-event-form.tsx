...
import { Event, type TicketType } from "@/types";
...
import { draftEvent, editEvent } from "@/actions/event-actions";
...
export default function EditEvent({
  ...
  const [pamphletUrl, setPamphletUrl] = useState<string>(event.pamphletUrl);
  const [formData, setFormData] = useState({
    ...
    eventEnd: event.eventEnd ? event.eventEnd : "",
    organizerName: event.organizerName,
    organizerEmail: event.organizerEmail,
    organizerPhone: event.organizerPhone,
    pamphletUrl: event.pamphletUrl,
  });
  ...
  const getTimeFromISOString = (isoString: string | null): string => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      ...
      } else if (key === "publishedAt" && value === null) {
    ...
    try {
      const res = await editEvent(eventId, formDataToSend);
      if (res.success) {
        toast({
          title: "Event Updated",
        ...
        if (res.error && typeof res.error === "object") {
          const nestedErrors = Object.entries(res.error)
            ...
            .join("\n");
          toast({
            title: "Error while editing the event",
                        ...
                        onSelect={(date) =>
                          handleDateTimeChange(
                            ...
                            getTimeFromISOString(
                              formData.eventEnd ? formData.eventEnd : null,
                            ),
                          )
                        }
                        initialFocus
              ...
              <>
                {formData.ticketTypes.map((ticket, index) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -20 }}
                    className="grid grid-cols-1 gap-4 rounded-lg border-b p-4 pb-4 shadow-sm last:border-b-0 md:grid-cols-5"
                  >
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        required
                        className={cn(
                          errors[`ticketTypes[${index}].seatsRemaining`]
                            ? "border-red-500"
                            : "",
                        )}
                      />
  ...
  const getStepDescription = (step: number) => {
    switch (step) {
      case 0:
        return "Update the core details of your event.";
      case 1:
        return "Adjust when your event will take place.";
      case 2:
        return "Update the organizer's contact information.";
      case 3:
        return "Modify the location details.";
      case 4:
        return "Update ticket options for your attendees.";
      case 5:
        return "Change the visual for your event.";
      case 6:
        return "Review your updated event pamphlet before saving.";
      default:
        return "";
    }
  };
  ...
  return (
    <div className="container mx-auto w-full max-w-5xl">
      ...
      >
        <h1 className="text-2xl font-extrabold sm:text-4xl">Edit Your Event</h1>
      </motion.div>
      <motion.div
